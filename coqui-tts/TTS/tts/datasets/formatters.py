...
def all_in_one(root_path, meta_file=None, **kwargs):  # pylint: disable=unused-argument
    """Normalizes the JLCorpus meta data file to TTS format
    """
    txt_file = "/home/zolkin/repo/Speech-Backbones/Grad-TTS/filelists/ru_stress/train_fixed.txt"
    items = []
    with open(txt_file, "r", encoding="utf-8") as ttf:
        for line in ttf:
            wav_fpath, _, text, speaker_name = line.strip().split("|")
            # wav_fpath = os.path.join(root_path, wav_fpath)
            items.append({"text": text, "audio_file": wav_fpath, "speaker_name": speaker_name, "root_path": root_path})
    return items


def wav_emb_text_spk(root_path, meta_file="metadata_pitch_stress_22k.csv", **kwargs):  # pylint: disable=unused-argument
    """Normalizes the JLCorpus meta data file to TTS format
    """
    txt_file = os.path.join(root_path, meta_file)
    items = []
